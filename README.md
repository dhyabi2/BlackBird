...
Pool custody uses 2-of-3 FROST (Ed25519, Blake2b-512) threshold signing
across three machines — no single machine ever holds the pool key, and key
generation is a true DKG (no trusted dealer). The Nano-compatible
ciphersuite is implemented first-party in `frost/src/suite.rs`. See
[`docs/FROST_MIGRATION.md`](docs/FROST_MIGRATION.md) for the deployment and
fund-migration runbook, `frost/` for the signing CLI, and
`src/vela_cosigner.py` for the verifying cosigner service.
...
The deployed instance runs the `t-of-n` FROST threshold guardian network from [`DESIGN.md`](DESIGN.md) with `t = 2, n = 3`: keys generated by dealerless DKG, funds custodied 2-of-3, and every signature co-verified independently by a second machine.
