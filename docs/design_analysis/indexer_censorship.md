...
| 3 | On-chain Nano anchoring | Anchor Merkle root hash in a Nano `RootCommit` block for tamper-evident timestamping. | False-root collusion. | Indexers include root hash in `RootCommit`. | Limited data space; not a slashing enforcement. | 80 |
...
| 10 | Fraud proofs on external smart-contract chain | Smart-contract fraud proofs for invalid Merkle proofs. | Invalid proofs passing client checks. | Cross-chain fraud-proof contract. | Gas costs; external-chain dependency; rejected. | 45 |

## Composite fix (selected)

**Combination:** Multi-indexer consensus + Merkle proof verification + on-chain Nano anchoring + IPFS proof availability + public audit bounties.

**Description:** Clients query multiple indexers and cross-check roots, then verify proofs locally. Roots are anchored on Nano via `RootCommit` transactions for tamper evidence. Proofs are replicated to IPFS for availability. Bounties (paid from a community fund or protocol treasury) incentivize detection of discrepancies.

**Score:** 92/100

## Author's note

Several of these ideas are already partially in VELA v2 (multi-indexer consensus, `RootCommit`, client verification). The remaining gaps are IPFS proof mirroring and bounty funding, which are documented as future hardening. No external smart contracts are used.
