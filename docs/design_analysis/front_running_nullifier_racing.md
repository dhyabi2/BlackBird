...
| 5 | Pure-Nano multi-sig bond slash for double-signing | Two conflicting signed withdrawals are published as a fraud proof; arbiters sign a Nano transaction to slash the bond. | Colluding guardians. | Multi-sig bond + arbiter process. | Requires honest arbiters; not automatic. | 72 |
...
**Combination:** Bind P_w into signed message + nullifier set check + first-confirmed-wins + pure-Nano multi-sig bond slash + optional withdrawal delay.

**Description:** Prevent coordinator front-running by binding `P_w` into the FROST-signed Nano block. Prevent nullifier racing by having guardians check the nullifier before signing and by relying on Nano's consensus to confirm one valid withdrawal first. Deter colluding guardians via a pure-Nano multi-sig bond slash and share revocation. Add an optional challenge window as a soft fallback.

**Score:** 92/100

## No external smart contracts

Double-signing is punished by arbiter-signed movement of a Nano multi-sig bond and by cryptographic share revocation, not by an external chain.
